...
#define HJMBMainThreadAssert() NSAssert([NSThread isMainThread], @"HJMBProgressHUD needs to be accessed on the main thread.");

CGFloat const HJMBProgressMaxOffset = 1000000.f;

static const CGFloat HJMBDefaultPadding = 4.f;
static const CGFloat HJMBDefaultLabelFontSize = 16.f;
static const CGFloat HJMBDefaultDetailsLabelFontSize = 12.f;


@interface HJMBProgressHUD ()
...
@interface HJMBProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    HJMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    HJMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (HJMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            HJMBProgressHUD *hud = (HJMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = HJMBProgressHUDAnimationFade;
    _mode = HJMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(HJMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == HJMBProgressHUDAnimationZoom) {
        type = animatingIn ? HJMBProgressHUDAnimationZoomIn : HJMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == HJMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == HJMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == HJMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == HJMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    HJMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<HJMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    HJMBBackgroundView *backgroundView = [[HJMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = HJMBProgressHUDBackgroundStyleSolidColor;
    ...
    HJMBBackgroundView *bezelView = [HJMBBackgroundView new];
    bezelView.translatesAutoresizingMaskIntoConstraints = NO;
    bezelView.layer.cornerRadius = 12.f;
    bezelView.alpha = 0.f;
    ...
    label.font = [UIFont boldSystemFontOfSize:HJMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:HJMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [HJMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:HJMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[HJMBRoundProgressView class]];

    HJMBProgressHUDMode mode = self.mode;
    if (mode == HJMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == HJMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[HJMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HJMBProgressHUDModeDeterminate || mode == HJMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[HJMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == HJMBProgressHUDModeAnnularDeterminate) {
            [(HJMBRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == HJMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HJMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[HJMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[HJMBProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[HJMBRoundProgressView class]]) {
        HJMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [HJMBRoundProgressView appearanceWhenContainedIn:[HJMBProgressHUD class], nil];
#else
        appearance = [HJMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[HJMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((HJMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((HJMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[HJMBBarProgressView class]]) {
        HJMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [HJMBBarProgressView appearanceWhenContainedIn:[HJMBProgressHUD class], nil];
#else
        appearance = [HJMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[HJMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((HJMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((HJMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    HJMBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? HJMBDefaultPadding : 0.f;
...
- (void)setMode:(HJMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == HJMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation HJMBRoundProgressView
...
@implementation HJMBBarProgressView
...
@implementation HJMBBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = HJMBProgressHUDBackgroundStyleBlur;
        if (@available(iOS 13.0, *)) {
            #if TARGET_OS_TV
            _blurEffectStyle = UIBlurEffectStyleRegular;
            #else
            _blurEffectStyle = UIBlurEffectStyleLight;
//            _blurEffectStyle = UIBlurEffectStyleSystemThickMaterial;//毛玻璃
            #endif
...
- (void)setStyle:(HJMBProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    HJMBProgressHUDBackgroundStyle style = self.style;
    if (style == HJMBProgressHUDBackgroundStyleBlur) {
        UIBlurEffect *effect =  [UIBlurEffect effectWithStyle:self.blurEffectStyle];
        UIVisualEffectView *effectView = [[UIVisualEffectView alloc] initWithEffect:effect];
        [self insertSubview:effectView atIndex:0];
        effectView.frame = self.bounds;
        effectView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        effectView.backgroundColor = [UIColor colorWithWhite:0 alpha:1];
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == HJMBProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation HJMBProgressHUDRoundedButton
